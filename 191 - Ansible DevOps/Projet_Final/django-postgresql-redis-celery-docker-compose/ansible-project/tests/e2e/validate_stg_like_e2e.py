...
def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit(f"DC13_VALIDATION_FAIL: {message}")
...
def submit_and_wait(
    ...
    while time.monotonic() < deadline:
        ...
        if task_status == "FAILURE":
            raise SystemExit(f"DC13_VALIDATION_FAIL: {label} task failed")
        time.sleep(1)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate the DC-13 STG-like Compose runtime.")
    parser.add_argument("--base-url", default="http://127.0.0.1:8081")
    parser.add_argument("--public-port", type=int, default=8081)
    ...
    print("DC13_HTTP_PASS: health/database/redis/celery")
    ...
    require(
        can_connect("127.0.0.1", args.public_port),
        f"public Nginx port {args.public_port} is not reachable",
    )
    print(f"DC13_NETWORK_PASS: 127.0.0.1:{args.public_port} reachable=true")
    ...
    for forbidden_port in (8000, 5432, 6379):
        ...
        print(f"DC13_NETWORK_PASS: 127.0.0.1:{forbidden_port} reachable=false")

    print("DC13_RUNTIME_VALIDATION_PASS")
    return 0
